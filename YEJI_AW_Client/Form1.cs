...
namespace YEJI_AW_Client
{
    public partial class Form1 : Form
    {
        ...
        private PcOffAlertForm? pcOffAlertForm;
        private UpdateProgressForm? updateProgressForm;
        private DateTime lastHeartbeatSuccessTime;
        ...
        private ToolStripSeparator? managerMenuSeparator;
        ...
        public Form1(string employeeName, string employeeId, HeartbeatWriter? heartbeatWriter = null)
        {
            ...
            this.computerIP = ApiClient.GetLocalIPAddress();
            _api = new ApiClient(employeeId ?? "", employeeName ?? "", computerName);
            ...
            // 서버 연결 상태 변경 시 트레이 툴팁 갱신
            ApiClient.ServerReachabilityChanged += (_, isReachable) =>
            {
                if (notifyIcon == null) return;
                if (InvokeRequired) { Invoke(() => UpdateTrayTooltip(isReachable)); return; }
                UpdateTrayTooltip(isReachable);
            };
        ...
        private async Task FetchWorkTimeFromServerAsync()
        {
            try
            {
                var client = ApiClient.Http;
                string url = ApiClient.BaseUrl + "/api/client-config";
                using var response = await client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                response.EnsureSuccessStatusCode();

                await using var stream = await response.Content.ReadAsStreamAsync();
                var workTimeInfo = await JsonSerializer.DeserializeAsync<WorkTimeInfo>(stream, ApiClient.JsonOpts);
        ...
        private async Task FetchProhibitedUrlsAsync()
        {
            try
            {
                ...
                // API 호출
                var client = ApiClient.Http;
                string url = $"{ApiClient.BaseUrl}/api/client/ban-urls?include_deleted=1";
                ...
                await using var stream = await response.Content.ReadAsStreamAsync();
                var apiResponse = await JsonSerializer.DeserializeAsync<BanUrlsResponse>(stream, ApiClient.JsonOpts);
        ...
        private async Task FetchBanItemExceptionsAsync()
        {
            ...
            try
            {
                ...
                string url = $"{ApiClient.BaseUrl}/api/client/ban-item-exceptions?employeeId={Uri.EscapeDataString(employeeId)}";
                using var response = await ApiClient.Http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                response.EnsureSuccessStatusCode();
                ...
                if (rowsElement.ValueKind == JsonValueKind.Array)
                {
                    ...
                    {
                        string type = ApiClient.GetJsonString(row, "type", "ban_type", "itemType");
                        string value = ApiClient.GetJsonString(row, "value", "itemValue", "url", "email");
                        if (string.IsNullOrWhiteSpace(type) || string.IsNullOrWhiteSpace(value))
                        {
                            continue;
                        }

                        string normalizedType = ApiClient.NormalizeBanItemType(type);
                        string normalizedValue = ApiClient.NormalizeBanItemValue(normalizedType, value);
                        if (string.IsNullOrWhiteSpace(normalizedValue))
                        {
                            continue;
                        }

                        parsed.Add(ApiClient.BuildBanExceptionKey(normalizedType, normalizedValue));
                    }
                }
        ...
        private bool IsBanItemException(string type, string? value)
        {
            ...
            if (banItemExceptionKeys.Contains(ApiClient.BuildBanExceptionKey(normalizedType, normalizedValue)))
            {
                return true;
            }
            ...
            foreach (string candidate in candidates)
            {
                ...
                if (banItemExceptionKeys.Contains(ApiClient.BuildBanExceptionKey(normalizedType, candidate)))
                {
                    return true;
                }
            }
        ...
        private ProhibitedUrlsCache? LoadProhibitedUrlsCache()
        {
            try
            {
                if (!File.Exists(LocalStore.BanUrlsCacheFile))
                {
                    return null;
                }

                string json = File.ReadAllText(LocalStore.BanUrlsCacheFile, Encoding.UTF8);
                return JsonSerializer.Deserialize<ProhibitedUrlsCache>(json, ApiClient.JsonOpts);
            }
            catch (Exception ex)
            {
        ...
        private void SaveProhibitedUrlsCache(ProhibitedUrlsCache cache)
        {
            try
            {
                string? folder = Path.GetDirectoryName(LocalStore.BanUrlsCacheFile);
                if (!string.IsNullOrEmpty(folder) && !Directory.Exists(folder))
                {
                    Directory.CreateDirectory(folder);
                }

                string json = JsonSerializer.Serialize(cache, ApiClient.JsonOpts);
                File.WriteAllText(LocalStore.BanUrlsCacheFile, json, Encoding.UTF8);
            }
            catch (Exception ex)
            {
        ...
        private async Task FetchProhibitedEmailsAsync()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    string url = $"{ApiClient.BaseUrl}/api/client/ban-emails?limit=50000";
                    if (!string.IsNullOrWhiteSpace(sinceParam))
                    {
                        url += $"&since={Uri.EscapeDataString(sinceParam)}";
                    }
                    ...
                    await using var stream = await response.Content.ReadAsStreamAsync();
                    var apiResponse = await JsonSerializer.DeserializeAsync<BanEmailsResponse>(stream, ApiClient.JsonOpts);
                    ...
                    var emailDict = cache.Emails.ToDictionary(e => ApiClient.NormalizeEmail(e.Email), e => e, StringComparer.OrdinalIgnoreCase);

                    if (apiResponse.Rows != null)
                    {
                        foreach (var row in apiResponse.Rows)
                        {
                            string normalizedEmail = ApiClient.NormalizeEmail(row.Email);
                    ...
                    prohibitedEmails = cache.Emails
                        .Select(e => ApiClient.NormalizeEmail(e.Email))
        ...
        private ProhibitedEmailsCache? LoadProhibitedEmailsCache()
        {
            try
            {
                if (!File.Exists(LocalStore.BanEmailsCacheFile))
                {
                    return null;
                }

                string json = File.ReadAllText(LocalStore.BanEmailsCacheFile, Encoding.UTF8);
                return JsonSerializer.Deserialize<ProhibitedEmailsCache>(json, ApiClient.JsonOpts);
            }
            catch (Exception ex)
            {
        ...
        private void SaveProhibitedEmailsCache(ProhibitedEmailsCache cache)
        {
            try
            {
                string? folder = Path.GetDirectoryName(LocalStore.BanEmailsCacheFile);
                if (!string.IsNullOrEmpty(folder) && !Directory.Exists(folder))
                {
                    Directory.CreateDirectory(folder);
                }
                ...
                string json = JsonSerializer.Serialize(cache, ApiClient.JsonOpts);
                File.WriteAllText(LocalStore.BanEmailsCacheFile, json, Encoding.UTF8);
            }
            catch (Exception ex)
            {
        ...
        private void CheckMailComposeRecipients(string currentUrl)
        {
            ...
            LogEmailDebug($"Extracted recipients: {string.Join(", ", foundEmails.Select(ApiClient.NormalizeEmail).Where(x => !string.IsNullOrWhiteSpace(x)))}");

            var matchedRows = new List<BanEmailRow>();
            foreach (string foundEmail in foundEmails)
            {
                string normalized = ApiClient.NormalizeEmail(foundEmail);
                ...
                var row = prohibitedEmailRows.FirstOrDefault(r =>
                    string.Equals(ApiClient.NormalizeEmail(r.Email), normalized, StringComparison.OrdinalIgnoreCase));
            ...
            string signature = string.Join(";", matchedRows.Select(m => ApiClient.NormalizeEmail(m.Email)).OrderBy(x => x));
        ...
        private void ShowProhibitedEmailAlert(List<BanEmailRow> matchedRows)
        {
            try
            {
                ...
                using var alertForm = new ProhibitedEmailAlertForm(matchedRows, browserWindowHandle);
                alertForm.TopMost = true;
                alertForm.ShowDialog();

                LogEmailDebug($"Prohibited email alert closed. matchedCount={matchedRows.Count}");

                string logEmails = string.Join(", ", matchedRows.Select(m => ApiClient.NormalizeEmail(m.Email)));
        ...
        private void SaveClientConfig(WorkTimeInfo info)
        {
            try
            {
                string? folder = Path.GetDirectoryName(LocalStore.ClientConfigFile);
                ...
                File.WriteAllText(LocalStore.ClientConfigFile, json, Encoding.UTF8);
            }
            catch
            {
                // 저장 실패는 조용히 무시
        ...
        private WorkTimeInfo? LoadClientConfig()
        {
            try
            {
                if (!File.Exists(LocalStore.ClientConfigFile))
                    return null;

                string json = File.ReadAllText(LocalStore.ClientConfigFile, Encoding.UTF8);
                var info = JsonSerializer.Deserialize<WorkTimeInfo>(json, ApiClient.JsonOpts);
                return info;
            }
            catch
            {
        ...
        private async Task CheckClientUpdateAsync(bool forceInstall = false, bool notifyWhenNoUpdate = false)
        {
            try
            {
                ...
                var checkResult = await _api.GetClientUpdateInfoAsync(currentVersion);
        ...
        private void ShowUpdateCompletionNotificationIfNeeded()
        {
            try
            {
                ...
                string folder = Path.GetDirectoryName(LocalStore.ClientVersionFile) ?? string.Empty;
                ...
                if (File.Exists(LocalStore.ClientVersionFile))
                {
                    previousVersion = File.ReadAllText(LocalStore.ClientVersionFile, Encoding.UTF8).Trim();
                ...
                if (File.Exists(LocalStore.PendingUpdateMarkerFile))
                {
                    pendingUpdateVersion = File.ReadAllText(LocalStore.PendingUpdateMarkerFile, Encoding.UTF8).Trim();
                ...
                File.WriteAllText(LocalStore.ClientVersionFile, currentVersion, Encoding.UTF8);
                ...
                if (hasPendingUpdateMarker)
                {
                    if (updatedFromPrevious || updatedFromPending)
                    {
                        File.Delete(LocalStore.PendingUpdateMarkerFile);
        ...
        private void UpdateTrayTooltip(bool isReachable)
        {
            if (notifyIcon == null) return;
            notifyIcon.Text = isReachable ? ApplicationName : $"{ApplicationName} [서버 연결 끊김]";
        }
        ...
        private async Task DownloadAndInstallUpdateAsync(ClientReleaseInfo latestRelease, bool bypassDelay = false)
        {
            ...
            ClientLogger.LogUpdate($"Current version: {currentVersion}");
            ...
            // 업데이트 진행창 표시
            updateProgressForm?.Close();
            updateProgressForm = new UpdateProgressForm(currentVersion, targetVersion);
            updateProgressForm.FormClosed += (_, _) => updateProgressForm = null;
            ...
            string downloadUrl = latestRelease.DownloadUrl.StartsWith("http", StringComparison.OrdinalIgnoreCase)
                ...
                : ApiClient.BaseUrl + latestRelease.DownloadUrl;
            ...
            updateProgressForm?.SetProgress("다운로드 준비 중...");
            for (int attempt = 1; attempt <= MaxUpdateDownloadRetries; attempt++)
            {
                try
                {
                    ...
                    using var response = await ApiClient.Http.GetAsync(downloadUrl, HttpCompletionOption.ResponseHeadersRead);
                    response.EnsureSuccessStatusCode();
                    ...
                    while ((bytesRead = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        {
                            int pct = (int)(downloaded * 100L / contentLength.Value);
                            string mb = $"{downloaded / 1048576.0:F1} / {contentLength.Value / 1048576.0:F1} MB 다운로드 중...";
                            updateProgressForm?.SetProgress(mb, pct);
                        }
                        else
                        {
                            updateProgressForm?.SetProgress($"{downloaded / 1048576.0:F1} MB 다운로드 중...");
                        }
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    updateProgressForm?.SetProgress($"다운로드 재시도 중... ({attempt}/{MaxUpdateDownloadRetries})");
                    await Task.Delay(TimeSpan.FromSeconds(waitSeconds));
                }
            }

            // 다운로드 완료 후 파일 존재 및 크기 검증
            updateProgressForm?.SetProgress("파일 검증 중...", 98);
        ...
        private void CreatePendingUpdateMarker(string targetVersion)
        {
            try
            {
                string folder = Path.GetDirectoryName(LocalStore.PendingUpdateMarkerFile) ?? string.Empty;
                ...
                File.WriteAllText(LocalStore.PendingUpdateMarkerFile, targetVersion, Encoding.UTF8);
        ...
        private bool HasRecentlyAttemptedUpdate(string version)
        {
            try
            {
                if (File.Exists(LocalStore.PendingUpdateMarkerFile))
                {
                    string pendingVersion = File.ReadAllText(LocalStore.PendingUpdateMarkerFile, Encoding.UTF8).Trim();
                    string currentVersion = GetCurrentVersion();
                ...
                if (!File.Exists(LocalStore.LastUpdateAttemptFile))
                {
                    return false;
                }

                var lines = File.ReadAllLines(LocalStore.LastUpdateAttemptFile, Encoding.UTF8);
                if (lines.Length < 2)
                {
                    return false;
                }
        ...
        private void RecordUpdateAttempt(string version)
        {
            try
            {
                string folder = Path.GetDirectoryName(LocalStore.LastUpdateAttemptFile) ?? string.Empty;
                ...
                File.WriteAllText(LocalStore.LastUpdateAttemptFile, content, Encoding.UTF8);
        ...
        private async Task<DateTime?> GetApprovedOvertimeEndTimeAsync(DateTime now)
        {
            try
            {
                ...
                var url = $"{ApiClient.BaseUrl}/api/overtime-requests?employeeId={Uri.EscapeDataString(employeeId)}&startDate={today}&endDate={today}";
                using var response = await ApiClient.Http.GetAsync(url);
                ...
                foreach (var entry in ApiClient.EnumerateArrayLike(root))
                {
                    string status = ApiClient.GetElementString(entry, "status", "approvalStatus", "approval_status", "result");
                    if (!string.Equals(status, "APPROVED", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }

                    string workDate = ApiClient.GetElementString(entry, "workDate", "work_date", "date");
                    ...
                    string startTime = ApiClient.GetElementString(entry, "startTime", "start_time", "start");
                    string endTime = ApiClient.GetElementString(entry, "endTime", "end_time", "end");
        ...
        private async Task<bool> HasActiveShutdownExceptionAsync()
        {
            try
            {
                var url = $"{ApiClient.BaseUrl}/api/shutdown-plan?employeeId={Uri.EscapeDataString(employeeId)}";
                using var response = await ApiClient.Http.GetAsync(url);
                if (!response.IsSuccessStatusCode)
                {
                    return false;
                ...
                bool shutdownDisabled = ApiClient.GetElementBool(root, "shutdownDisabled", "shutdown_disabled");
                string mode = ApiClient.GetElementString(root, "mode", "planMode", "plan_mode");
        ...
        private async Task ShowPcOffAlertAsync(DateTime now, DateTime offTime, bool triggeredAfterBoot, bool isFollowUpAlert, bool isOvertimeAlert = false)
        {
            ...
            pcOffAlertForm.TempDisableClicked += (s, e) =>
            {
                ...
                pcOffAlertForm?.OnTempDisableFailed($"{StandardShutdownCountdownMinutes}분 후 PC가 강제종료됩니다.");
            };

            pcOffAlertForm.FormClosed += (s, e) => pcOffAlertForm = null;
        ...
        {
            var remaining = scheduledShutdownTime.HasValue
                ? scheduledShutdownTime.Value - GetCurrentDateTime()
                : TimeSpan.Zero;
            if (remaining < TimeSpan.Zero) remaining = TimeSpan.Zero;

            pcOffAlertForm?.UpdateCountdown(remaining, remainingTempDisableCount);
        ...
        private async Task ShowPopupAsync(PopupSchedule popup)
        {
            ...
            var popupImage = await LoadScaledImageAsync(ApiClient.BaseUrl + popup.ImageUrl, maxImageSize.Width, maxImageSize.Height);
            if (popupImage == null)
            {
        ...
        private async Task EnsurePcOffSettingsAsync()
        {
            ...
            try
            {
                string url = $"{ApiClient.BaseUrl}/api/pc-off-settings";
                using var response = await ApiClient.Http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                if (response.IsSuccessStatusCode)
                {
                    await using var stream = await response.Content.ReadAsStreamAsync();
                    var settings = await JsonSerializer.DeserializeAsync<PcOffSettings>(stream, ApiClient.JsonOpts);
                    if (settings != null)
                    {
        ...
        private int LoadUsedTempDisableCountForToday()
        {
            try
            {
                if (!File.Exists(LocalStore.TempDisableStateFile))
                {
                    return 0;
                }

                string json = File.ReadAllText(LocalStore.TempDisableStateFile, Encoding.UTF8);
                var state = JsonSerializer.Deserialize<TempDisableUsageState>(json, ApiClient.JsonOpts);
                if (state == null)
                {
                    return 0;
                }
        ...
        private void SaveUsedTempDisableCountForToday(int usedCount)
        {
            try
            {
                string? folder = Path.GetDirectoryName(LocalStore.TempDisableStateFile);
                ...
                string json = JsonSerializer.Serialize(state, ApiClient.JsonOpts);
                File.WriteAllText(LocalStore.TempDisableStateFile, json, Encoding.UTF8);
            }
            catch
            {
                // 저장 실패 시 무시
        ...
        private string ResolveImageUrl(string imageUrl)
        {
            ...
            return $"{ApiClient.BaseUrl.TrimEnd('/')}/{imageUrl.TrimStart('/')}";
        }
        ...
        private async Task<List<PopupSchedule>> FetchPopupSchedulesAsync()
        {
            ...
            {
                var client = ApiClient.Http;
                string url = ApiClient.BaseUrl + "/api/scheduled-popups";
                using var response = await client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                response.EnsureSuccessStatusCode();

                await using var stream = await response.Content.ReadAsStreamAsync();
                var popups = await JsonSerializer.DeserializeAsync<List<PopupSchedule>>(stream, ApiClient.JsonOpts)
        ...
        private async Task ShowIdleReasonPopupAsync(DateTime start, DateTime end)
        {
            ...
            try
            {
                ...
                using IdleReasonForm form = new IdleReasonForm(start, end, ApiClient.BaseUrl);
        ...
        {
            computerIP = ApiClient.GetLocalIPAddress();
        ...
        private async Task<ClientStatusResult> PostClientStatusAsync(string url)
        {
            ...
            var endpoint = ApiClient.TryGetEndpoint(url);

            try
            {
                ClientLogger.LogWeb($"Posting client status to {endpoint} for {payload.EmpNo}/{payload.PcName}.", "DBG");

                var response = await ApiClient.Http.PostAsync(url, content);
        ...
        private void LogClientStatusIssue(string context, Exception? ex = null, HttpResponseMessage? response = null, string? body = null)
        {
            try
            {
                string? folder = Path.GetDirectoryName(LocalStore.ClientStatusLogFile);
                if (!string.IsNullOrWhiteSpace(folder))
                {
                    Directory.CreateDirectory(folder);
                }
                ...
                sb.AppendLine();
                File.AppendAllText(LocalStore.ClientStatusLogFile, sb.ToString(), Encoding.UTF8);
        ...
        private async Task<bool> SendIdleEventAsync(IdleEventData data)
        {
            ...
            {
                var client = ApiClient.Http;
                string url = $"{ApiClient.BaseUrl}/api/idle-events";
        ...
        private async Task SendPcEventAsync(string eventType)
        {
            ...
            {
                var client = ApiClient.Http;
                string url = $"{ApiClient.BaseUrl}/api/pc-events";
        ...
        private async Task SendHeartbeatAsync()
        {
            ...
            try
            {
                ...
                var result = await PostClientStatusAsync($"{ApiClient.BaseUrl}/api/client/heartbeat");
                if (result.Success) lastHeartbeatSuccessTime = DateTime.Now;
        ...
        {
            if (!File.Exists(LocalStore.PendingIdleEventsFile))
            {
                return;
            }
            ...
            try
            {
                var json = await File.ReadAllTextAsync(LocalStore.PendingIdleEventsFile, Encoding.UTF8);
                var pending = JsonSerializer.Deserialize<List<IdleEventData>>(json, ApiClient.JsonOpts) ?? new List<IdleEventData>();
                if (pending.Count == 0)
                {
                    File.Delete(LocalStore.PendingIdleEventsFile);
                    return;
                }
                ...
                else
                {
                    ...
                    await File.WriteAllTextAsync(LocalStore.PendingIdleEventsFile, updatedJson, Encoding.UTF8);
                }
            }
            catch
            {
        ...
        private async Task RegisterOrUpdateClientAsync()
        {
            try
            {
                var result = await PostClientStatusAsync($"{ApiClient.BaseUrl}/api/client/register");
                if (result.UserNotFound)
                {
                    ScheduleUserInfoReentryPrompt();
                }
        ...
        private async Task CheckAndAddManagerMenuAsync()
        {
            try
            {
                ...
                string url = $"{ApiClient.BaseUrl}/api/client/manager-info?employeeId={Uri.EscapeDataString(emp)}";
                using var response = await ApiClient.Http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                if (!response.IsSuccessStatusCode)
                    return;

                await using var stream = await response.Content.ReadAsStreamAsync();
                var mgr = await JsonSerializer.DeserializeAsync<ManagerInfoResponse>(stream, ApiClient.JsonOpts);
        ...
        private void EnableManagerNotificationMenu()
        {
            ...
            if (managerMenuSeparator != null)
                managerMenuSeparator.Visible = true;
        ...
        private void UpdateManagerNotificationMenuLabel(int newCount)
        {
            ...
            // 배지 이미지 갱신 (기존 이미지 dispose → 새 이미지 설정)
            var oldImage = managerNotificationsMenuItem.Image;
            managerNotificationsMenuItem.Image = CreateNotificationBadge(newCount);
            oldImage?.Dispose();
        }

        private static Bitmap? CreateNotificationBadge(int count)
        {
            ...
            using var bgBrush = new SolidBrush(UiTheme.Danger);
            g.FillEllipse(bgBrush, 0, 0, W - 1, H - 1);
            string text  = count > 99 ? "99+" : count.ToString();
            float  fSize = count > 9 ? 6.5f : 7.5f;
            using var font      = new Font("Segoe UI", fSize, FontStyle.Bold);
        ...
        private async Task<List<ManagerNotificationItem>> FetchManagerNotificationsAsync()
        {
            ...
            try
            {
                string url = $"{ApiClient.BaseUrl}/api/client/manager-notifications?employeeId={Uri.EscapeDataString(employeeId)}";
                using var response = await ApiClient.Http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                if (!response.IsSuccessStatusCode)
                {
                    return list;
        ...
        private List<ManagerNotificationItem> ParseManagerNotifications(string json)
        {
            ...
            try
            {
                ...
                foreach (var item in ApiClient.EnumerateArrayLike(source))
                {
                    var overtime = item;
                    ...
                    {
                        Id = ApiClient.GetElementString(item, "id", "_id", "notificationId"),
                        NotificationStatus = ApiClient.GetElementString(item, "notificationStatus", "status"),
                        OvertimeRequest = new OvertimeRequestSummary
                        {
                            Id = ApiClient.GetElementString(overtime, "id", "_id", "requestId", "request_id"),
                            EmployeeId = ApiClient.GetElementString(overtime, "employeeId", "employee_id", "empNo", "emp_no"),
                            EmployeeName = ApiClient.GetElementString(overtime, "employeeName", "employee_name", "empName", "emp_name"),
                            WorkDate = ApiClient.GetElementString(overtime, "workDate", "work_date", "date"),
                            StartTime = ApiClient.GetElementString(overtime, "startTime", "start_time", "start"),
                            EndTime = ApiClient.GetElementString(overtime, "endTime", "end_time", "end"),
                            Reason = ApiClient.GetElementString(overtime, "reason", "description", "comment"),
                            Status = ApiClient.GetElementString(overtime, "status", "approvalStatus", "approval_status", "result"),
                        }
                    });
                }
            }
        ...
        private async Task OpenManagerNotificationsAsync(IEnumerable<string>? notificationIdsToMark)
        {
            ...
            try
            {
                ...
                managerNotificationListForm = new ManagerNotificationListForm(ApiClient.BaseUrl, ApiClient.Http,employeeId, employeeName, notificationIdsToMark);
        ...
        private async Task<List<EmployeeOvertimeRequest>> FetchEmployeeOvertimeRequestsAsync()
        {
            ...
            try
            {
                ...
                string url = $"{ApiClient.BaseUrl}/api/overtime-requests?employeeId={Uri.EscapeDataString(employeeId)}&startDate={startDate}&endDate={endDate}";
                using var response = await ApiClient.Http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead);
                if (!response.IsSuccessStatusCode)
                {
                    return list;
                ...
                foreach (var item in ApiClient.EnumerateArrayLike(root))
                {
                    var request = new EmployeeOvertimeRequest
                    {
                        Id = ApiClient.GetElementString(item, "id", "requestId", "request_id"),
                        WorkDate = ApiClient.GetElementString(item, "workDate", "work_date", "date"),
                        StartTime = ApiClient.GetElementString(item, "startTime", "start_time", "start"),
                        EndTime = ApiClient.GetElementString(item, "endTime", "end_time", "end"),
                        Reason = ApiClient.GetElementString(item, "reason", "description", "comment"),
                        Status = ApiClient.GetElementString(item, "status", "approvalStatus", "approval_status", "result"),
                        Approver = ApiClient.GetElementString(item, "approver", "approverName", "approvedBy", "approved_by", "approver_name")
                    };
        ...
        private async void OnViewIdleHistory(object? sender, EventArgs e)
        {
            ...
            if (hasHistoryViewPermission)
            {
                onViewOrgHistory = () =>
                {
                    var managedForm = new ManagedIdleHistoryForm(ApiClient.BaseUrl, ApiClient.Http,employeeId);
        ...
        private async Task<List<IdleEventData>> FetchIdleHistoryForDateRangeAsync(string targetEmpId, DateTime fromDate, DateTime toDate)
        {
            ...
            {
                var client = ApiClient.Http;
                string url = $"{ApiClient.BaseUrl}/api/idle-events?employeeId={Uri.EscapeDataString(targetEmpId)}&startDate={fromDate:yyyy-MM-dd}&endDate={toDate:yyyy-MM-dd}";
                string json = await client.GetStringAsync(url);
                return JsonSerializer.Deserialize<List<IdleEventData>>(json, ApiClient.JsonOpts) ?? new List<IdleEventData>();
            }
            catch
            {
        ...
        private void OnShowClientStatus(object? sender, EventArgs e)
        {
            var form = new ClientStatusForm(
                version:         GetCurrentVersion(),
                employeeId:      employeeId,
                employeeName:    employeeName,
                computerName:    computerName,
                isServerReachable: ApiClient.IsServerReachable,
                lastHeartbeat:   lastHeartbeatSuccessTime,
        ...
        private void OnOpenOvertimeRequest(object? sender, EventArgs e)
        {
            ...
            var form = new OvertimeRequestForm(ApiClient.BaseUrl, ApiClient.Http,employeeId, GetCurrentDateTime);
            ShowTrayMenuForm(form);
            form.Dispose(); // ShowDialog() is synchronous, so this happens after the form closes
        }
        ...
        {
            var form = new OvertimeRequestListForm(ApiClient.BaseUrl, ApiClient.Http,employeeId);
            ShowTrayMenuForm(form);
            form.Dispose(); // ShowDialog() is synchronous, so this happens after the form closes
        }
        ...
        {
#if DEBUG
            var form = new ShutdownExceptionListForm(ApiClient.BaseUrl, ApiClient.Http,employeeId);
            ShowTrayMenuForm(form);
            form.Dispose(); // ShowDialog() is synchronous, so this happens after the form closes
#endif
        }
        ...
        private void InitializeTrayMenu()
        {
            ...
            trayMenu.Items.Add("연장근무 신청", null, OnOpenOvertimeRequest);
            trayMenu.Items.Add("연장근무 신청 확인", null, OnOpenOvertimeStatus);

            // ── 설정 ─────────────────────────────────────────────────
            trayMenu.Items.Add(new ToolStripSeparator());
            trayMenu.Items.Add("사용자 정보 수정", null, OnEditUserInfo);
            trayMenu.Items.Add("클라이언트 정보", null, OnShowClientStatus);

            // ── 관리자 메뉴 (기본 숨김, EnableManagerNotificationMenu에서 표시) ──
            managerMenuSeparator = new ToolStripSeparator { Visible = false };
            managerNotificationsMenuItem = new ToolStripMenuItem(
                "연장근무승인 결재", null,
                async (s, e) => await OpenManagerNotificationsAsync(null))
            {
                Visible = false
            };
            trayMenu.Items.Add(managerMenuSeparator);
            trayMenu.Items.Add(managerNotificationsMenuItem);

#if DEBUG
            // ── 디버그 서브메뉴 ───────────────────────────────────────
            trayMenu.Items.Add(new ToolStripSeparator());
            var debugMenu = new ToolStripMenuItem("디버그");
            debugMenu.DropDownItems.Add("자리비움 사유 창 열기",  null, OnDebugOpenIdleReason);
            debugMenu.DropDownItems.Add("PC오프 알림 확인",       null, OnDebugShowPcOffAlert);
            debugMenu.DropDownItems.Add("연장 근무 신청 창 열기", null, OnDebugOpenOvertimeRequest);
            debugMenu.DropDownItems.Add("연장 근무 결과 확인",    null, OnDebugOpenOvertimeStatus);
            debugMenu.DropDownItems.Add("PC 종료 예외 조회",      null, OnDebugOpenShutdownExceptions);
            debugMenu.DropDownItems.Add("현재 시각 모의 설정",    null, OnDebugSetCurrentTime);
            debugMenu.DropDownItems.Add("현재 시각 모의 해제",    null, OnDebugClearCurrentTime);
            debugMenu.DropDownItems.Add("업데이트 테스트 실행",   null, OnDebugRunUpdateTest);
            debugMenu.DropDownItems.Add("관리자 진단",            null, async (s, e) => await CheckAndAddManagerMenuAsync());
            debugMenu.DropDownItems.Add("관리자 알림 확인",       null, async (s, e) => await CheckManagerNotificationsAsync(forceShowPopup: true));
            debugMenu.DropDownItems.Add("직원 연장근무 알림",     null, async (s, e) => await CheckEmployeeOvertimeStatusAsync());
            trayMenu.Items.Add(debugMenu);
#endif
        ...
        private void SavePendingIdleEvent(IdleEventData data)
        {
            try
            {
                Directory.CreateDirectory(Path.GetDirectoryName(LocalStore.PendingIdleEventsFile)!);
                ...
                File.WriteAllText(LocalStore.PendingIdleEventsFile, json, Encoding.UTF8);
            }
            catch (Exception ex)
            {
                // 로컬 저장 실패 시 로그 기록
        ...
        private List<IdleEventData> LoadPendingIdleEvents()
        {
            try
            {
                ...
                var json = File.ReadAllText(LocalStore.PendingIdleEventsFile, Encoding.UTF8);
                return JsonSerializer.Deserialize<List<IdleEventData>>(json, ApiClient.JsonOpts) ?? new List<IdleEventData>();
            }
            catch
            {
